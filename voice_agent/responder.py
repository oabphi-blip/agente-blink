...
def _agenda_block(ctx: Optional[dict]) -> str:
    ...
    C-105 (11/08/2026): quando ctx.known.slots_selecionados está presente
    (pré-selecionados por Python em enriquecimento_ctx step 11), exibe
    APENAS esses slots com instrução imperativa. LLM não escolhe mais —
    só formata o texto em volta dos slots fornecidos.
    ...
    # ── C-105: usa slots pré-selecionados por Python (NÃO expõe agenda inteira) ──
    slots_sel = known.get("slots_selecionados")
    if slots_sel:
        try:
            from voice_agent.oferta_slot_deterministico import formatar_slots_para_prompt
            medico = ctx.get("medico") or known.get("medico") or ""
            unidade = known.get("unidade") or ""
            return formatar_slots_para_prompt(slots_sel, medico=medico, unidade=unidade)
        except Exception:
...
def _gerar_oferta_3_slots(ctx: Optional[dict]) -> str:
    ...
    C-105 (11/08/2026): usa ctx.known.slots_selecionados (pré-selecionados
    por Python em enriquecimento_ctx step 11) quando disponível. LLM não
    escolhe — Python já decidiu.
    ...
    # C-105: usa slots pré-selecionados por Python se disponível
    tres = known.get("slots_selecionados") or _selecionar_3_slots_para_oferta(agenda, turno_pref)
    if not tres:
        return (
