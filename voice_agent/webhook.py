...
def create_app(settings: Optional[Settings] = None) -> FastAPI:
    ...
    def _process_kommo(
        ...
        # Bug C-100 (10/08/2026) — Salesbot path (/kommo) não tinha
        # agent_paused_for_lead. Enquanto _process_whatsapp_cloud verificava
        # ATIVADO IA e status_id antes de responder, _process_kommo chamava
        # responder.reply() direto — Lia respondia mesmo com lead em
        # 1-ATENDIMENTO HUMANO ou ATIVADO IA = Desativado.
        # Fix: espelhar a checagem do caminho WhatsApp Cloud.
        if pipeline.kommo is not None and caller_context:
            try:
                ...
                log.warning("/kommo agent_paused_for_lead falhou lead=%s: %s", lead_id, e)
                motivo = None
            if motivo:
                log.info(
                    "[C-100] /kommo agente em silêncio (%s) para lead %s — "
                    "postando agent_answer vazio no return_url",
                    motivo, lead_id,
                )
                # Carimba DESATIVADO pra manter consistência com o caminho WA Cloud.
                lid = caller_context.get("lead_id") or (int(lead_id) if lead_id else None)
                if lid:
                    try:
                        pipeline.kommo.update_lead_fields(
                            int(lid), {"ativado_ia": "DESATIVADO"}
                        )
                    except Exception as e:  # noqa: BLE001
                        log.warning("/kommo C-100 carimbo ATIVADO IA? (off) falhou: %s", e)
                # Posta agent_answer vazio no return_url para o Salesbot não ficar
                # pendurado esperando resposta. Com agent_answer="", o Salesbot
                # não envia nada ao paciente — silêncio correto.
                if return_url:
                    _headers = {"Content-Type": "application/json"}
                    if settings.kommo_token:
                        _headers["Authorization"] = f"Bearer {settings.kommo_token}"
                    try:
                        with httpx.Client(timeout=10) as _c:
                            _c.post(
                                return_url,
                                json={"data": {"agent_answer": ""}},
                                headers=_headers,
                            )
                    except Exception as e:  # noqa: BLE001
                        log.warning("/kommo C-100 return_url silêncio falhou: %s", e)
                return
