# TIPO DE PAGAMENTO — AGENDAMENTO EXCLUSIVO / ENCAIXE (EXCLUSIVO DRA. KARLA DELALÍBERA)

## 🚨 ATENÇÃO
Este roteiro é **estritamente EXCLUSIVO** e deve ser utilizado **somente** para consultas e procedimentos **particulares** (sem convênio, incluindo SDP) com a **Dra. Karla Delalíbera**.

O Agente deve utilizá-lo na **fase de finalização do agendamento logístico**, enviando **estritamente um passo por vez** e aguardando a resposta antes de avançar para a próxima etapa.

---

## 🔵 PASSO 1 — DEFINIÇÃO DA UNIDADE (aguardar resposta)

```
Excelente escolha, [Nome do Paciente]! Para darmos o primeiro passo na
organização da sua consulta com a Dra. Karla Delalíbera, qual das nossas
unidades fica mais confortável para você?

1️⃣ Asa Norte
2️⃣ Águas Claras
```

🚨 **O Agente obrigatoriamente aguarda a resposta.**

---

## 🔵 PASSO 2 — APRESENTAÇÃO DOS MODELOS DE AGENDAMENTO (CONTRAS DE VALOR) (aguardar resposta)

```
Perfeito! Para garantirmos o melhor fluxo para o seu cuidado e respeitarmos
o tempo de todos os pacientes, trabalhamos com duas opções de agendamento
na nossa clínica:

1️⃣ Reserva Exclusiva Imediata (com adiantamento de 50%)
O adiantamento garante a reserva imediata e a exclusividade do seu dia e
horário exatos na agenda.

2️⃣ Fila de Encaixe Perfeito (sem adiantamento)
O pagamento é feito apenas no dia da consulta. Registramos suas preferências
e, assim que surgir uma vaga ou desistência que corresponda à sua
disponibilidade, nossa equipe avisa para realizar o encaixe.

Qual formato você prefere para seguirmos? ✨
```

🚨 **O Agente obrigatoriamente aguarda a resposta.**

---

## 🔵 PASSO 3 — COLETA DE PREFERÊNCIAS (BIFURCAÇÃO LOGÍSTICA POR UNIDADE) (aguardar resposta)

**Nota ao Agente:** Verifique a unidade escolhida no Passo 1 e envie **apenas as opções correspondentes** abaixo.

### 🔴 Se a unidade escolhida for ASA NORTE

```
Tudo compreendido! Como a nossa agenda na Asa Norte é bem dinâmica,
quais são as suas preferências para buscarmos o seu horário ideal?

1️⃣ Qual dia prefere? (Segunda, Quarta ou Sexta)
2️⃣ Qual turno prefere? (Manhã ou Tarde)
3️⃣ Qual período do turno prefere? (Início, Meio ou Fim)
```

### 🔵 Se a unidade escolhida for ÁGUAS CLARAS

```
Tudo compreendido! Como a nossa agenda em Águas Claras é bem dinâmica,
quais são as suas preferências para buscarmos o seu horário ideal?

1️⃣ Qual dia prefere? (Terça ou Quinta)
2️⃣ Qual turno prefere? (Manhã, Tarde ou Início da Noite)
3️⃣ Qual período do turno prefere? (Início, Meio ou Fim)
```

🚨 **O Agente obrigatoriamente aguarda a resposta com as preferências.**

---

## 🔵 PASSO 4 — ENCERRAMENTO E TRANSFERÊNCIA (finalização)

```
Tudo perfeitamente anotado! ✅

Vou passar agora para nossa Concierge Especialista, Rafaela Ortiga,
que assumirá o contato em breve para os próximos passos da sua reserva. ✨

📋 Resumo do atendimento:
👤 Paciente: [Nome]
🏥 Modalidade: Particular (Dra. Karla) — [Reserva Imediata 50% OU Fila de Encaixe]
📍 Preferência: [Unidade] — [Dias, Turnos e Períodos escolhidos]
```

---

## ⚠️ Regras críticas

1. **Apenas Dra. Karla particular** — não usar para Dr. Fabrício ou convênios.
2. **Um passo por vez** — esperar resposta a cada passo.
3. **Reserva 50%:** chave Pix Asa Norte: `karladelaliberaoftalmo@gmail.com` | Águas Claras: CNPJ `52.303.729/0001-30`.
4. **Fila de Encaixe:** sem adiantamento, paga no dia.
5. **Concierge Rafaela Ortiga** assume a partir do Passo 4.
