...
## 🎯 OBJETIVO
Acolher a dúvida do paciente que pergunta sobre convênio na **primeira mensagem**, sem confirmar cobertura antes de identificar o motivo (regra crítica por causa de Avaliação do Processamento Visual e Lentes de Prisma, que são exclusivamente sem convênio).
...
Avaliação do Processamento Visual e Lentes de Prisma **não têm cobertura** — confirmar convênio antes do motivo gera falsa expectativa.
...
1. **Verifique se o caso é EXCEÇÃO** (Avaliação do Processamento Visual ou Prisma): atendimento exclusivamente sem convênio, **NÃO** consultar lista de convênios. → ir para artigo 31 (Avaliação do Processamento Visual) ou 14 (sem convênio).
