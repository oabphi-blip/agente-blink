...
# Bug C-101 (10/08/2026) — Pediátrico → Karla
# Detecta criança/bebê no texto → injeta medico="Karla" sem perguntar
_PEDIATRIC_KW = re.compile(
    r"\b(?:beb[eê]|crian[çc]a|crian[çc]as|beb[eê]s|infante|rec[eé]m[- ]?nascido"
    r"|meu\s+filho|minha\s+filha|meu\s+beb[eê]|minha\s+beb[eê]"
    r"|para\s+(?:(?:meu|minha|um|uma|o|a)\s+)?(?:beb[eê]|crian[çc]a|filho|filha|rec[eé]m[- ]?nascido))\b",
    re.IGNORECASE,
)
# "para X anos" / "de X anos" / "com X anos" — extrai idade numérica
_PEDIATRIC_AGE = re.compile(
    r"\bpara\s+(?:(?:\w+\s+){0,3}?)(\d{1,2})\s+anos?\b"
    r"|\bde\s+(\d{1,2})\s+anos?\b"
    r"|\bcom\s+(\d{1,2})\s+anos?\b"
    r"|\b(\d{1,2})\s+anos?\s+de\s+(?:idade|vida)\b",
    re.IGNORECASE,
)
# "X meses" → sempre bebê → Karla
_PEDIATRIC_MESES = re.compile(r"\b\d{1,2}\s+meses?\b", re.IGNORECASE)
...
def _extract_pre_slots(text: str) -> PreSlots:
    ...
    # --- Bug C-101: Pediátrico → Karla Delalíbera ---
    # Quando médico não foi mencionado explicitamente, tenta detectar criança/bebê.
    # ORDEM CRÍTICA: idade numérica tem prioridade sobre keywords.
    # "meu filho de 45 anos" → 45 ≥ 18 → adulto (não injeta Karla).
    if slots.medico is None:
        _m_age = _PEDIATRIC_AGE.search(text)
        if _m_age:
            # Idade encontrada: decide por ela, ignora keywords
            _age_str = _m_age.group(1) or _m_age.group(2) or _m_age.group(3) or _m_age.group(4)
            if _age_str and int(_age_str) < 18:
                slots.medico = "Karla"
                log.debug("[C-101] pré-extração: idade %s anos → medico=Karla", _age_str)
            # Se ≥18: não injeta Karla — adulto mesmo que "filho" apareça
        elif _PEDIATRIC_MESES.search(text):
            slots.medico = "Karla"
            log.debug("[C-101] pré-extração: bebê em meses → medico=Karla")
        elif _PEDIATRIC_KW.search(text):
            slots.medico = "Karla"
            log.debug("[C-101] pré-extração: criança/bebê detectado → medico=Karla")
