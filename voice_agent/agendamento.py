...
def executar_agendamento(
    ...
    # ── C-111 (11/08/2026): RE-VERIFICAR SLOT ANTES DE GRAVAR (anti race-condition) ──
    # Lia ofertou o slot horas atrás; outro paciente pode ter ocupado entretanto.
    # Verificar AGORA antes de chamar criar_agendamento.
    # Fail-open: se Medware falhar na checagem, prossegue (não bloqueia conversa).
    _data_iso_c111 = decision.get("data_iso", "")
    _hora_c111 = decision.get("hora", "")
    _medico_nome_c111 = decision.get("medico", "")
    _unidade_nome_c111 = decision.get("unidade", "")
    if _data_iso_c111 and _hora_c111 and hasattr(medware, "slot_ainda_disponivel"):
        try:
            _slot_ok, _slots_alt = medware.slot_ainda_disponivel(
                data_iso=_data_iso_c111,
                hora=_hora_c111,
                cod_medico=cod_medico or 12080,
                cod_unidade=cod_unidade or 5,
                medico_nome=_medico_nome_c111,
                unidade_nome=_unidade_nome_c111,
            )
            if not _slot_ok:
                log.warning(
                    "[C-111] RACE CONDITION: slot %sT%s já ocupado lead=%s — devolvendo alternativas",
                    _data_iso_c111, _hora_c111, (caller_context or {}).get("lead_id"),
                )
                # Formatar até 2 alternativas para re-oferta pela Lia
                _emojis = ["1️⃣", "2️⃣"]
                _linhas = []
                for _i, _s in enumerate(_slots_alt[:2]):
                    _d_br = _s.get("data_br") or _s.get("data_iso", "")
                    _dia_s = _s.get("dia_semana", "")
                    _h_s = _s.get("hora", "")
                    if _d_br and _h_s:
                        _dia_fmt = f"{_dia_s} ({_d_br})" if _dia_s else _d_br
                        _linhas.append(f"{_emojis[_i]} {_dia_fmt} às {_h_s}")
                if _linhas:
                    _msg_alt = (
                ...
                return {
                    "ok": False,
                    "motivo": "slot_ocupado_race_condition",
                    "msg_para_paciente": _msg_alt,
                    "slots_alternativos": _slots_alt[:2],
                }
        except Exception as _e_c111:
            log.warning("[C-111] re-verificação de slot falhou (fail-open): %s", _e_c111)
