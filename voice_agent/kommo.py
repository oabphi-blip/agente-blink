    ...
    def get_caller_context_by_lead(self, lead_id: int | str) -> dict:
        ...
        try:
            ...
            id_to_label = {
                ...
                # Bug C-102 (11/08/2026) — data de nascimento do paciente.
                # Usado por enriquecimento_ctx.py para calcular idade e derivar
                # medico=Karla automaticamente quando paciente < 18 anos.
                # Campo `date` (não date_time) — valor retorna como ISO string
                # "YYYY-MM-DDT00:00:00-03:00". Zero custo extra (mesmo GET acima).
                FIELD_DATA_NASCIMENTO_PACIENTE_1: "data_nasc",
            ...
            for cf in (data.get("custom_fields_values") or []):
                fid = cf.get("field_id")
                # Bug C-109 (11/08/2026) — NO-SHOW COUNT: leitura por nome de campo
                # (field_id não confirmado via API — busca por field_name é mais robusta
                # contra recriação do campo). Política: >=2 → sinal 50%; >=3 → humano.
                _cf_fname = (cf.get("field_name") or "").upper().strip()
                if _cf_fname in ("NO-SHOW COUNT", "NO-SHOW", "NOSHOW COUNT",
                                 "N.NO-SHOW", "NOSHOW", "NO SHOW COUNT"):
                    _ns_vals = cf.get("values") or []
                    if _ns_vals:
                        try:
                            out["known"]["noshow_count"] = int(
                                _ns_vals[0].get("value") or 0
                            )
                        except (ValueError, TypeError):
                            pass
                    continue
